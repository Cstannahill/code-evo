...
class CodeQualityAnalysis(BaseModel):
    ...
    testability: float = Field(ge=0.0, le=100.0, description="Testability score")
    issues_found: List[str] = Field(description="List of quality issues")
    recommendations: List[str] = Field(description="Improvement recommendations")


class EvolutionAnalysis(BaseModel):
    """Code evolution analysis result"""
    evolution_trend: str = Field(description="Overall evolution trend")
    complexity_change: str = Field(description="Complexity change over time")
    refactoring_frequency: float = Field(description="Refactoring frequency")
    technical_debt: float = Field(ge=0.0, le=100.0, description="Technical debt score")
...
class SecurityAnalysis(BaseModel):
    ...
    vulnerabilities: List[Dict[str, Any]] = Field(description="Security vulnerabilities found")
...
class ArchitecturalAnalysis(BaseModel):
    ...
    architectural_debt: float = Field(ge=0.0, le=100.0, description="Architectural debt score")
...
class AnalysisRequest(BaseModel):
    ...
    commit_limit: int = Field(default=100, ge=1, le=1000, description="Maximum commits to analyze")
    ...
    include_architecture: bool = Field(default=True, description="Include architectural analysis")
...
class AnalysisResponse(BaseModel):
    ...
    progress: float = Field(ge=0.0, le=100.0, description="Analysis progress percentage")
    results: Optional[ComprehensiveAnalysisResult] = Field(default=None, description="Analysis results")
    error_message: Optional[str] = Field(default=None, description="Error message if analysis failed")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Analysis start time")
...
class ModelSelectionRequest(BaseModel):
    ...
    analysis_type: AnalysisType = Field(description="Type of analysis to perform")
    repository_url: Optional[str] = Field(default=None, description="Repository URL for analysis")
    branch: str = Field(default="main", description="Branch to analyze")
    commit_limit: int = Field(default=50, ge=1, le=1000, description="Maximum commits to analyze")
...
class EnsembleMetadata(BaseModel):
    ...
    ensemble_score: float = Field(ge=0.0, le=1.0, description="Ensemble confidence score")


class IncrementalAnalysisMetadata(BaseModel):
    """Metadata for incremental analysis"""
    previous_analysis_id: Optional[str] = Field(default=None, description="Previous analysis ID")
    files_changed: List[str] = Field(description="Files that changed since last analysis")
    ...
    time_saved: float = Field(ge=0.0, description="Time saved compared to full analysis")
...
class AnalysisConfig(BaseModel):
    ...
    max_commits: int = Field(default=100, ge=1, le=1000, description="Maximum commits to analyze")
    max_files: int = Field(default=1000, ge=1, le=10000, description="Maximum files to analyze")
    timeout_seconds: int = Field(default=300, ge=30, le=3600, description="Analysis timeout")
...
class ModelConfig(BaseModel):
    ...
    max_tokens: int = Field(default=4000, ge=100, le=32000, description="Maximum tokens")
