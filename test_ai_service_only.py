...
async def _async_test_ai_service_directly():
    """Async implementation of the AIService model selection test"""
    ...
    try:
        ...
        print(
            f"   OpenAI models available: {status.get('openai_models_available', False)}"
        )
        ...
        print(
            f"✅ Quality analysis: {quality_result.get('model_used', 'unknown')} model"
        )
...
def test_ai_service_directly():
    """Synchronous pytest entrypoint that runs the async test"""
    import asyncio

    asyncio.run(_async_test_ai_service_directly())
